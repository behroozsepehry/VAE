...
class Model(normal_base.VaeModelNormalBase):
    def __init__(self, **kwargs):
        ...
        self.fc2 = nn.Sequential(nn.Linear(self.z_dim, 2 * 2 * 32),
                                 nn.ReLU(True),
                                 )

        self.deconv = nn.Sequential(
            nn.ConvTranspose2d(32, 32, 3, stride=2),  # b, 16, 5, 5
            nn.ReLU(True),
            nn.ConvTranspose2d(32, 16, 5, stride=3, padding=1),  # b, 8, 15, 15
            nn.ReLU(True),
        )
        self.deconv_mu = nn.Sequential(nn.ConvTranspose2d(16, 1, 2, stride=2, padding=1),  # b, 1, 28, 28
            nn.Sigmoid(),
        )
        self.deconv_logvar = nn.Sequential(nn.ConvTranspose2d(16, 1, 2, stride=2, padding=1),  # b, 1, 28, 28
        )
    ...
    def decode(self, *args, **kwargs):
        assert len(args) == 1
        z, = tuple(args)
        h2 = self.fc2(z).view(z.size(0), 32, 2, 2)
        h3 = self.deconv(h2)
